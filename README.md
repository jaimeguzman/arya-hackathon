...
> **`local/`** — team sandbox / Phase-style demo implementation. On this branch it may be gitignored for day-to-day work; when present, see `local/README.md` / `local/PROJECT.md`. The **canonical product story and architecture for the hackathon** are the root docs above.
...
| Doc | Why open it |
|-----|-------------|
| [`PROJECT.md`](./PROJECT.md) | Problem, agent design, feature plan, official brief |
| [`must-have.md`](./must-have.md) | Safety checks judges will notice |
| [`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md) | Mermaid sequences + pipeline layers |
| [`WORKFLOW.md`](./WORKFLOW.md) | Plain-English happy paths + edge cases |
| [`data/README.md`](./data/README.md) | ICD-10 subset, payers, sample referrals, licensing |
| [`feature_list.json`](./feature_list.json) | Ranked feature inventory |
| [`CLAUDE.md`](./CLAUDE.md) / [`WORKFLOW.md`](./WORKFLOW.md) | Contributor / agent workflow |

---

## Demo narrative (judges)

1. **Inbound provider call** — agent answers, collects referral facts, mid-call eligibility.  
2. **Fax/PDF** — upload a packet; pipeline extracts fields + gaps.  
3. **Follow-up** — missing F2F / docs trigger outbound or SMS.  
4. **Family call** — compassionate intake without clinical overreach.  
5. **Decline path** — out-of-area / uncovered case returns clear reasons.  
6. **Dashboard** — show status, confidence, gaps, and live call state.

---

## Hackathon constraints

- Build and demo during the sprint  
- **Must use Twilio** for telephony (sponsor prizes)  
- Ground answers in domain knowledge; personalize by caller/referral source  
- Prefer reliability and guardrails over feature sprawl  

---

## License / data note

Synthetic and reference datasets under `data/` are for demo only — see [`data/README.md`](./data/README.md) for sources and licensing. Do not commit secrets (`.env`).
