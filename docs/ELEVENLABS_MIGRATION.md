...
| `apis/api_intake/app/routes/twilio.py` | `apis/api_elevenlabs/` (new encapsulated API folder per the API encapsulation rule) reusing the mode router, intake agents, and safety layer from `api_intake` via clear imports or an internal package — decide the seam in Task 1 before coding |
