...
- **ElevenLabs is the telephony/voice platform.** TEAM DECISION 2026-07-11 (see [`docs/ELEVENLABS_MIGRATION.md`](docs/ELEVENLABS_MIGRATION.md), also recorded in [`PROJECT.md`'s Team Decision Log](PROJECT.md#️-team-decision-log-known-deviation-from-the-brief-2026-07-11)): the project migrates from Twilio to ElevenLabs Agents (inbound + outbound voice; follow-ups via outbound calls instead of SMS), knowingly forgoing Twilio sponsor-prize eligibility. This supersedes the original "Twilio is mandatory" rule in the organizers' brief — do not re-introduce Twilio without an explicit new team decision. The brief text in `PROJECT.md` stays intact as the historical record; the migration doc governs. **Scope note**: the migration targets `apis/api_elevenlabs/` (new). `local/` is an explicit workspace boundary out of scope for this migration — its existing Twilio/ConversationRelay code stays as-is until reconciled at merge day.
...
- All other `.md` files ([`must-have.md`](must-have.md), [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md), [`WORKFLOW.md`](WORKFLOW.md), [`data/README.md`](data/README.md), anything under `docs/`, future feature docs) support and extend `PROJECT.md`. Before implementing anything, verify the proposed work aligns with `PROJECT.md`.
...
- [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) is the diagram-and-steps reference (Mermaid diagrams, numbered flows, proposed module layout for file ownership) derived from `PROJECT.md`. If it and `PROJECT.md` disagree, `PROJECT.md` wins — fix `docs/ARCHITECTURE.md` to match, not the reverse.
- [`WORKFLOW.md`](WORKFLOW.md) is the plain-English, step-by-step narration of both entry channels (voice call, fax) for onboarding — it restates `docs/ARCHITECTURE.md`'s diagrams in prose, it doesn't add new decisions. If it drifts from `docs/ARCHITECTURE.md`, fix `WORKFLOW.md`.
